...
check_status(){
   ...
   do
       ...
       status_mds=`yrfs-ctl --listtargets --nodetype=meta --state|awk 'NR>2'|grep -v Good|wc -l`

       #status_oss=`yrcli --osd --type=oss|grep -v "up/clean"|awk 'NR>1'|wc -l`
       status_oss=`yrfs-ctl --listtargets --nodetype=storage --state|awk 'NR>2'|grep -v Good|wc -l`
