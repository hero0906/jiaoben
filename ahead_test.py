...
md5 = "md5sum " + filename
node_ip = "192.168.45.14"
kernel_cmd = "uname -a|awk '{print $3}'"
cache_mode = "cat /etc/yrfs/yrfs-client.conf|grep tuneFile|awk 'NR==1{print $3}'"

kernel = commands.getoutput(kernel_cmd)
cache = commands.getoutput(cache_mode)

log_path = os.path.join(os.getcwd(), kernel + "_" + cache)
...
def fio_run(bs, rw, numjobs):
 ...
 -bs={1} -rw={2} -numjobs={3} -time_based -iodepth=1 -output={4}/fio_{1}{2}{3}.log".format(filename, bs, rw, numjobs,\
         log_path)
    return fio_cmd
...
def dd_run(bs, rw):
    if rw == "read":
        if bs in "4K, 4k":
            ...
            count = 10000
        dd_cmd = "dd if={0} of=/dev/null bs={1} count={2} status=progress 2>>{4}/dd_{3}{1}.log".format(filename, bs,\
                count, rw, log_path)
    if rw == "write":
        if bs in "4K, 4k":
            count = 10000000
            ...
            count = 50000
        dd_cmd = "dd if=/dev/zero of={0} bs={1} count={2} status=progress 2>>{4}/dd_{3}{1}.log".format(filename, bs,\
                count, rw, log_path)
    return dd_cmd
...
def md5_test():
   ...
   with open("%s/md5.log" % log_path, "a") as f:
